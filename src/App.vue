...
const STAT_NAMES = ["Weight", "Acceleration", "On-Road Traction", "Off-Road Traction", "Mini-Turbo", "Ground Speed", "Water Speed", "Anti-Gravity Speed", "Air Speed", "Ground Handling", "Water Handling", "Anti-Gravity Handling", "Air Handling", "Invincibility"]
...
<template>
  ...
  <div v-if="mode == 'Closest'">
    <h2>Source build</h2>
    <v-select label="Character" v-model="source_character" :items=CHARACTER_LIST style="display: inline-block; width: 25%; overflow: hidden; text-overflow: ellipsis;"></v-select>
    <v-select label="Vehicle" v-model="source_vehicle" :items=VEHICLE_LIST style="display: inline-block; width: 25%; overflow: hidden; text-overflow: ellipsis;"></v-select>
    <v-select label="Wheels" v-model="source_wheels" :items=WHEEL_LIST style="display: inline-block; width: 25%; overflow: hidden; text-overflow: ellipsis;"></v-select>
    <v-select label="Glider" v-model="source_glider" :items=GLIDER_LIST style="display: inline-block; width: 25%; overflow: hidden; text-overflow: ellipsis;"></v-select>
    <h2>Stats to consider</h2>
    <v-checkbox v-for="(stat, index) in STAT_NAMES" v-model="stats_to_consider[index]" @change="update_stats_to_consider(index)" :label=stat color="primary"></v-checkbox>
  </div>
  <div>
    <h2>Constraints</h2>
    <v-expansion-panels>
      <v-expansion-panel>
        ...
        <v-expansion-panel-text>
          <v-checkbox v-for="(character, index) in CHARACTER_LIST" @change="recalculate()" v-model="allowed_characters" :label=character :value=index color="primary"></v-checkbox>
        </v-expansion-panel-text>
      </v-expansion-panel>
      <v-expansion-panel>
        <v-expansion-panel-title>
        ...
        <v-expansion-panel-text>
          <v-checkbox v-for="(vehicle, index) in VEHICLE_LIST" @change="recalculate()" v-model="allowed_vehicles" :label=vehicle :value=index color="primary"></v-checkbox>
        </v-expansion-panel-text>
      </v-expansion-panel>
      <v-expansion-panel>
        <v-expansion-panel-title>
        ...
        <v-expansion-panel-text>
          <v-checkbox v-for="(wheel, index) in WHEEL_LIST" @change="recalculate()" v-model="allowed_wheels" :label=wheel :value=index color="primary"></v-checkbox>
        </v-expansion-panel-text>
      </v-expansion-panel>
      <v-expansion-panel>
        <v-expansion-panel-title>
        ...
        <v-expansion-panel-text>
          <v-checkbox v-for="(glider, index) in GLIDER_LIST" @change="recalculate()" v-model="allowed_gliders" :label=glider :value=index color="primary"></v-checkbox>
        </v-expansion-panel-text>
      </v-expansion-panel>
    </v-expansion-panels>
  </div>
  <div>
    <v-text-field @change="recalculate()" v-model="num_of_builds" type="number" label="Number of builds to generate" style="width:300px"></v-text-field>
  </div>
  <div>
  <div>
    <v-card v-if="mode=='Closest'">
      <v-card-title>{{ source_build }}</v-card-title>
      <v-card-text>
